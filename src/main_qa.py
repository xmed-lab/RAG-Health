...
from dataset import get_task_fn, convert_dataset, generate_patient_group, re_construct_format, \
    get_map_system, create_sft_data_flask, re_construct_format_think, create_sft_data_think_flask, create_sft_data_infer_flask, re_construct_format_infer,create_sft_data_flash_flask, re_construct_format_flash
...
from qa_parse import MedQADataset, preprocess_medqa, split_data_qa,presplit_qa, CommonQADataset, preprocess_common
...
def run_single_config(config, exp_num='3'):
    ...
    if not os.path.exists(root_to + 'datasets_pre_stand.pkl'):
        ...
        if config['DATASET'] == 'MedQA':
            base_dataset = MedQADataset(
                root="/hpc2hdd/home/xxxs349/xxxc/RAGHealth/data/raw/qa_dataset/medqa",
                table='medqa_train_1000'
            )
            base_dataset = base_dataset.get_data()
            preprocess = preprocess_medqa
        elif config['DATASET'] in ['afrimedqa', 'pubmedqa', 'mmlu-pro', 'mmlu', 'medxpertqa', 'medqa', 'medmcqa', 'medexqa', 'medbullets']:
             base_dataset  = CommonQADataset(
                root="/nfs/scratch/xxxc/DATA/RAGHealth/data/{}/ready/".format(config['DATASET']),
                table=config['DATASET'])
             base_dataset = base_dataset.get_data()

             preprocess = preprocess_common
        else:
        ...
        try:
            ...
            if os.path.exists(root_to + 'train_dataset.pkl'):
                ...
                if config['DATASET']=='MedQA':
                    ...
                    test_dataset = presplit_qa('/hpc2hdd/home/xxxs349/xxxc/RAGHealth/data/raw/qa_dataset/medqa/phrases_no_exclude_test.jsonl')
                    val_dataset = test_dataset.copy()
                elif config['DATASET'] in ['afrimedqa', 'pubmedqa', 'mmlu-pro', 'mmlu', 'medxpertqa', 'medqa', 'medmcqa', 'medexqa', 'medbullets']:
                    train_dataset, test_dataset = samples['train'], samples['test']
                    val_dataset = ['empty']
                else:
                    train_dataset, val_dataset, test_dataset = split_data_qa(
...
if __name__ == '__main__':
    ...
    train = True # 必须先train保证数据处理，模型训练完成
    ...
    if train:
        ...
        if config['MODEL']== 'ours':
            test_data_path = root_to + 'test_sft_data.jsonl' # 看一下sft的
