...
class UNIFYCONFIG(): # 不要有drugs
    ...
    LLM_PATH = "/nfs/scratch/czhaobo/huggingface/hub/qwen25-7B"
    T_LLM = 'qwen25-7B'#'qwen-plus'#'deepseek-v3'#'LLAMA3-8B' # Teacher LLM是更强的LLMs; 统一调用API; 'LLAMA3-3B'#除了qwen都要改
    T_LLM_PATH =  "/nfs/scratch/czhaobo/huggingface/hub/qwen25-7B" #''#'/home/czhaobo/huggingface/hub/llama3-8B/' # 注意最后一个下划线。vllm server挂载和python 不一致
    EMB = 'E5' # 一般不要换，不然得重新构建embedding。
    EMB_PATH = "/nfs/scratch/czhaobo/huggingface/hub/e5-v2"
    ...
    TOPK = 1 # entity
    DEPTH = 5
    MODEL_PATH = "/nfs/scratch/czhaobo/huggingface/hub/qwen25-7B" #'/home/czhaobo/RAGHealth/LLaMA-Factory/output/llama3_lora_reward' # final model path； 需要最后统一下名字
    # statistic
    ...
    # log
    LOGDIR = '/home/czhaobo/RAGHealth/log/ckpt/'
