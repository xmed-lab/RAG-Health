...
def get_normalize_text(preds, golds, config):
    ...
    if config['TASK'] in ['MOR', 'REA', 'IHM']:
        ...
        default_answer = 0 # [1] +[0]*9 # 默认值, 这里非常危险
        preds = normalize_answer(preds)
        golds = normalize_answer(golds)
        # scalr_gold = [0] * 10
...
######### for llm generation ##########
#
# def llm_generation(query, large_llm='deepseek-r1', system_instruction='', api_base=''):
#     """感觉使用API划算点, 要么就使用VLLM挂服务"""
#     if large_llm in ['qwen2.5-72b-instruct', 'deepseek-r1']:
#         api_key = 'sk-fa3ff964b0e54be0ae4f9e65dfef2a4e'
#         client = OpenAI(
...
#         )
#
#         tmp = completion.model_dump_json()
#         result_text = completion.choices[0].message.content
#         print(result_text)
#         return result_text
#     elif large_llm == 'vllm': # 本地挂载
#         if api_base == '':
#             raise ValueError("Please provide the base url for the local API")
#
...
#
#         # Completion API 调用
#         completion = client.completions.create(model=model, prompt=query)
...
def get_llm_model(config, llm_name, api_base=None, path=None, device=None):
    ...
    if llm_name in ['LLAMA3-1B']: # 用于调试环境, 单卡； 也可以使用Ollama部署服务。; 注意，不支持动态参数修改操作
        ...
        #     model=model,
        #     tokenizer=tokenizer,
        #     device=device,  # 如果有GPU，可以使用 device=0
        #     return_full_text=False, # 避免query一起返回
        #     max_new_tokens=config['run_config_dict']['max_tokens'],  # 设置生成文本的最大token数
        #     temperature=config['run_config_dict']['temperature'],  # 可以设置其他参数如temperature
        #     # top_p=0.9,  # 以及top_p等参数
        # )
        ...
        DASHSCOPE_API_KEY = 'sk-fa3ff964b0e54be0ae4f9e65dfef2a4e'
        ...
        # 指定模型
    elif llm_name in ['baichuan-32B', 'qwen25-7B','LLAMA3-8B', 'qwen25-32B', 'deepseekr1-7B', 'meditron-7B', 'biomistral-7B', 'LLAMA3-3B', 'qwq-32B']:
        # 设置 OpenAI API 密钥和 API 基础地址
        if api_base is None:
            print("========Please check vllm service is running========")
            api_base = "http://localhost:8020/v1"  # 本地默认服务地址
...
def copy_file(src, dst, root_to=None, test_mode=False):
    ...
    # 复制文件
    if root_to is None:
        root_to = '/home/czhaobo/RAGHealth/LLaMA-Factory/data/'
    else:
        root_to = root_to
...
if __name__ == '__main__':
    ...
    #     openai_api_key=openai_api_key,
    #     openai_api_base=openai_api_base,
    #     model_name='/home/czhaobo/huggingface/hub/llama3-8B/',
    #     # model_kwargs={"stop": ["."]},
    # )
    # # 假设 llm 已经正确初始化
    ...
    # from langchain_core.output_parsers import StrOutputParser  # 导入字符串输出解析器
    #
    aliyun_api_key = "sk-fa3ff964b0e54be0ae4f9e65dfef2a4e"  # 替换为你的阿里云API Key
    llm = ChatOpenAI(
    ...
    # system_instruction = ""
    # meta_path = ["Patient-has_diagnosis-Disease-affects-Anatomical_Structure", "Laboratory_Test-measures-Biomarker-elevated_in-Disease", "Disease-precedes-Disease-diagnosed_by-Diagnostic_Test"]
    # query = """
    ...
    # """
    #
    # llm_generation(query=query, system_instruction=system_instruction, large_llm='qwen2.5-72b-instruct')


    #
    # import dashscope
    #
    # dashscope.api_key = "sk-fa3ff964b0e54be0ae4f9e65dfef2a4e"
    # response = dashscope.Generation.call(
    #     model="deepseek-v3",  # 通义千问模型
    #     prompt="请介绍一下量子计算"
    # )
    # print(response.output.text)  # 直接输出字符串