...
warnings.filterwarnings(
    "ignore",
    message=r"Trainer.tokenizer is now deprecated. You should use Trainer.processing_class instead."
)
...
class CustomPPOTrainer(PPOTrainer, Trainer):
    ...
    def __init__(
        ...
        backward_batch_size = training_args.per_device_train_batch_size * training_args.gradient_accumulation_steps # overall batchsize
        ...
        self.generation_config = GenerationConfig( # seveal generation settings
        ...
        if finetuning_args.reward_model_type == "full":
            if self.is_deepspeed_enabled:
                if not (
                    getattr(reward_model.pretrained_model, "is_loaded_in_8bit", False) # 模型加载
        ...
        for step in tqdm(range(max_steps), disable=not self.is_local_process_zero()):
            ...
            for idx in range(0, self.config.batch_size, self.config.mini_batch_size):
                mini_batch_queries, mini_batch_responses = self.get_inputs(
                    batch[idx : idx + self.config.mini_batch_size] # r"""Generate model's responses given queries. 这里PPO的reference model???."""
                )
            ...
            if self.is_local_process_zero() and (step + 1) % self.args.logging_steps == 0: # 如果是本地进程零并且达到记录步骤
                logs = dict(
    ...
    def get_rewards(
        ...
        """
        if self.finetuning_args.reward_model_type == "api": # 服务器端返回reward
        ...
        if self.finetuning_args.reward_model_type == "lora":
            replace_model(unwrapped_model, target="reward") # unwraped model指的是从封装模型中直接读取模型。
            reward_model = self.model # 使用当前模型为奖励模型
        else:
            reward_model = self.reward_model # 使用指定的奖励模型
        ...
        return rewards.float().detach()  # use fp32 type,获取奖励
