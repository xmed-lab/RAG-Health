...
DEFINED_CONFIG = {
    "dataset": "MIV",
    "TASK": "REA",
    "dataset_dir": "/hpc2hdd/home/xxxs349/xxxc/RAGHealth/",
    "USE_CUDA": True,
    "GPU": "0",
    "EMB": 'E5',  # 要改一起改
...
class HealthPPOTrainer(PPOTrainer, Trainer):
    ...
    def index_graph(self):
        # 2. 创建异构图 & text chunk
        root_to_dir = "/hpc2hdd/home/xxxs349/xxxc/RAGHealth/data/ready"  #
    ...
    def ppo_train(self, resume_from_checkpoint: Optional[str] = None) -> None:  # TODO: 重点要改
        ...
        for step in tqdm(range(max_steps), disable=not self.is_local_process_zero()):
            ...
            batch_token_with_mask_responses = self.trans_text_to_token(responses,use_template=False)
            queries, responses = self.get_inputs(batch_token_with_mask_queries,
                                                 batch_token_with_mask_responses)

            print("*********Start Step************")
            self.config.batch_size = len(queries)
        ...
        for reason_history in reason_historys:  # [[[{}]], [[{}]]]
            ...
            for one_reason in reason_history:  # 某一个deep think,# {}
                ...
                if one_reason['source'] == 'LLM':
                    ...
                    one_metapaths = one_reason['chosen_metapaths']
                    ...
                    duplicated_ids.append(list(duplicates))
